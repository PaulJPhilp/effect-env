...
export class ValidationError {
    constructor(missing, invalid, report) {
 ...
 */
export const formatValidationReport = (error) => {
    const lines = ["Key       | Status    | Details"];
    lines.push("-----------|-----------|--------");
 ...
 */
export const validate = (schema, source, opts) => {
    const shouldFail = opts?.failInProd ?? false;
    return S.decodeUnknown(schema)(source).pipe(Effect.asVoid, Effect.catchAll((parseError) => {
        const missing = [];
        const invalid = [];
        ...
        if (currentKey) {
            ...
            if (isMissing) {
                ...
                const meaningfulLine = errorDetails.find(d => d.includes('Expected') || d.includes('actual') || d.includes('failure')) || errorDetails[errorDetails.length - 1] || 'validation failed';
                invalid.push({
                    key: currentKey,
        ...
        const report = formatValidationReport(new ValidationError(missing, invalid, errors));
